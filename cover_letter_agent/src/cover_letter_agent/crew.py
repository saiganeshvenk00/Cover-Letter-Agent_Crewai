...
class CoverLetterCrew():
    ...
    def cover_letter_writer_agent(self) -> Agent:
        return Agent(
            config=self.agents_config['cover_letter_writer_agent'], # type: ignore[index]
            verbose=True
        )
    ...
    def cover_letter_task(self) -> Task:
        return Task(
            config=self.tasks_config['cover_letter_task'], # type: ignore[index]
            output_file='output/cover_letter.md'
        )

    @crew
    def crew(self) -> Crew:
        """Creates the CoverLetterAgent crew"""
